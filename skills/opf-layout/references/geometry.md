...
Shared content cards are published in core 0.10.0 and later. Pass the effective `design.contentBox` as the `contentBox` compose option; a slide's explicit `false` overrides a deck's `true`. Each body leaf's optional `frameBox` preserves its outer allocation, while `box` and accepted payload internals use a rounded interior padded by 12 reference pixels at a 720-pixel short edge (capped at one quarter of either frame dimension). Candidate scoring, overflow and pagination use that same interior. Headings and group/track geometry do not acquire another card inset. Consumers must draw the card at `frameBox` and reuse `box`; adding renderer-only padding would measure a different layout. Use the coordinated published set (core 0.11.0, renderer 0.9.0, PPTX 0.9.1, editor 0.8.0); core 0.9.0 predates this behavior.
