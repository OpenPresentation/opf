...
Default heading geometry is a reference-engine policy, not extra JSON. Title and subtitle share the padded width. Cover layouts (`title`, `title-subtitle`) and heading-only slides center the heading group in the remaining safe area; wrapping recenters the group. Content slides keep headings at the top and reserve two title line-heights at the requested title size before body content so typical one- and two-line titles do not move the body. Missing subtitle and tag fields leave no gap. Reuse `item.box` (and accepted internal part boxes) for preview, selection and export.
