...
Default heading geometry is a reference-engine policy, not extra JSON. Title and subtitle share the padded width. Cover layouts (`title`, `title-subtitle`) and heading-only slides center the heading group in the remaining safe area; wrapping recenters the group. Content slides keep headings at the top and reserve two title line-heights at the requested title size before body content so typical one- and two-line titles do not move the body, unless that unused space would shrink a 3×3 region cell below stacked metric metadata. Missing subtitle and tag fields leave no gap. Reuse `item.box` (and accepted internal part boxes) for preview, selection and export. Do not silently truncate; report overflow.
...
The same unpublished branch adds [shared vector-outline placement](https://github.com/OpenPresentation/opf/blob/codex/shared-metric-integration-20260910/docs/plans/text-placement.md) and `grid-score-v6`. Optional `textMeasurement.outlineBounds` and `textRasterPadding` determine accepted heading/scalar/rich line origins; pass identical options to pagination, preview, editing and export. Explanations report outline availability and effective padding. This does not measure every payload's ink or establish native raster equivalence.

The unpublished `codex/shared-metric-integration-20260910` branch adds shared content cards and `grid-score-v5`. Pass the effective `design.contentBox` as the `contentBox` compose option; a slide's explicit `false` overrides a deck's `true`. Each body leaf's optional `frameBox` preserves its outer allocation, while `box` and accepted payload internals use a rounded interior padded by 12 reference pixels at a 720-pixel short edge (capped at one quarter of either frame dimension). Candidate scoring, overflow and pagination use that same interior. Headings and group/track geometry do not acquire another card inset. Consumers must draw the card at `frameBox` and reuse `box`; adding renderer-only padding would measure a different layout. This behavior requires the coordinated source branches and is not available in the published core 0.9.0 set.
