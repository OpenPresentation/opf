...
This is an unreleased coordinated source change. Published core 0.4.1, renderer 0.2.0, PPTX 0.2.1 and editor 0.1.2 do not form a supported installation for rich table arrays.
...
Native PPTX table import still flattens runs to strings. Native PowerPoint rendering, cross-engine interaction, real OS IME, merged cells, cell fills/borders/alignment and advanced table controls remain unverified or unimplemented. Next release work must bump package versions and core dependency minimums, pin the coordinated CI sources, and verify packed and registry installations before describing rich tables as published.
