...
| Rich text runs | Every run field editable as structured data | Shared mixed-run font/style/color/link/script measurement and SVG/native PPTX output implemented for text payloads; native SVG selection toolbar and range replacement implemented; continuous mixed-style typing, mixed-script shaping and cross-engine caret calibration remain |
| Lists and bullets | Shared rich-run/description measurement, hanging indents, nesting, inline string editing, range formatting, structural controls and native PPTX bullets | Image bullets, continuous rich typing, lossless rich-list PPTX import and native raster parity |
...
- `/canvas-tests.html`: includes right-aligned caret overlay regression. `/rich-text-tests.html` covers 31 selection/formatting/range replacement checks with loaded fonts.
- Renderer suite: 126 corpus decks plus new design-preview tests, fonts, Office substitutes, browser font loader. Historical PNG goldens target an older OPF commit and remain separately gated.
