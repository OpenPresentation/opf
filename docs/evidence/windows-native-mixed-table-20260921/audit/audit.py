...
def main() -> int:
    ...
    check(
        "whole-cell text, five exact native runs, seven character starts/styles, one paragraph/tab",
        exact_content_pass,
    ...
    # Preserve COM float values and coordinates exactly as reported; do not infer their cause.
    ...
    check(
        "native geometry/tab/paragraph observations retained as reported numeric values",
        True,
        {"slidePoints": {"width": observation["slideWidth"], "height": observation["slideHeight"]},
         "tableShapeGeometryPoints": shape_geometry, "cellShapeGeometryPoints": cell_geometry,
         "wholeTextRangeBoundsPoints": whole_bounds, "paragraph": paragraph,
         "tabProbeGeometry": tab_evidence,
         "limitation": "TextRange2 bounds are reported alongside cell-shape geometry; this audit does not infer their coordinate origin or engine cause."},
    )
    ...
    result = {
        ...
        "scope": "Raw native-mixed-table-01 evidence, source/snapshot hashes, stages, registration records, line ranges, and source PPTX ZIP/XML. No Office/COM, font registration, UI, source edit, or Git operation was performed by this audit.",
