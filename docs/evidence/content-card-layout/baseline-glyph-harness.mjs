...
try{
 ...
 async function inspect(svg,expected,id){
  ...
  const measured=await page.evaluate(expected=>{
   ...
   }).filter(record=>expected?record.path===expected.path&&record.text===expected.text&&Object.keys(record.box).every(key=>Math.abs(record.box[key]-expected.box[key])<.05):true);
   if(candidates.length!==1)throw Error(`Expected one exact audited leaf, got ${candidates.length}`);
   const {node,...record}=candidates[0];node.setAttribute('data-glyph-probe','target');
  ...
  if(expected){assert.equal(measured.fill,expected.fill);assert.equal(measured.fontFamily,expected.fontFamily);assert.equal(measured.fontSize,expected.fontSize);}
  ...
  const threshold=expected?.threshold??(measured.fontSize>=24||(measured.fontWeight>=700&&measured.fontSize>=56/3)?3:4.5);
 ...
 for(const [index,record]of audit.failures.entries()){
  ...
  const auditedSlide=audit.decks.find(item=>item.file===record.file)?.slides.find(item=>item.slide===record.slide);assert.ok(auditedSlide);assert.equal(hash(svg),auditedSlide.svgSha256);
  const result=await inspect(svg,record,`finding-${String(index).padStart(2,'0')}`);assert.equal(JSON.stringify(deck),before);
  results.push({...result,file:record.file,slide:record.slide,path:record.path,text:record.text,sourceSha256:hash(source),svgSha256:hash(svg)});
 }
 assert.equal(results.length,audit.failures.length);assert.deepEqual(errors,[]);assert.deepEqual(requests,[]);
