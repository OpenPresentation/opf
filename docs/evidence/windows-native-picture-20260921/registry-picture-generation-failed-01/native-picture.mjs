...
for (const name of ['@openpresentation/opf', '@openpresentation/opf-render', '@openpresentation/opf-pptx', '@openpresentation/opf-editor']) {
  ...
  bindings[name] = {version: pkg.version, packageFile, publicEntry: resolve.resolve(name), resolved: locked.resolved, integrity: locked.integrity, files};
}
const {validatePresentation} = await import(pathToFileURL(resolve.resolve('@openpresentation/opf')));
const {toPptx, fromPptx} = await import(pathToFileURL(resolve.resolve('@openpresentation/opf-pptx')));
const {renderSvg, svgToPng} = await import(pathToFileURL(resolve.resolve('@openpresentation/opf-render')));
