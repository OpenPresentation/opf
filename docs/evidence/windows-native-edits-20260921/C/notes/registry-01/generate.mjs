...
for (const variant of ['original', 'reordered']) await mkdir(path.join(root, variant));
...
for (const {id, source, options = {}} of cases) {
  ...
  const reordered = zipSync(Object.fromEntries(Object.entries(entries).sort(([a], [b]) => a.localeCompare(b)).map(([name, value]) => [name, [value, {mtime: new Date(1980, 0, 1)}]])), {level: 6});
  ...
  assert.equal(changedXml.replace(match, '$2$1'), changedXml, 'The candidate no longer contains the invalid pair.');
  ...
  manifest.push({id, originalSha256: hash(bytes), reorderedSha256: hash(reordered), changedParts: differences, semanticImportSha256: hash(JSON.stringify(originalImport))});
}
...
console.log(`Generated ${cases.length} original/reordered pairs. Only presentation.xml differs; semantic imports match. No native acceptance is claimed.`);
