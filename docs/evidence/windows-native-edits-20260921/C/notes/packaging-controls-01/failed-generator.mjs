...
const root=path.dirname(fileURLToPath(import.meta.url)), out=path.join(root,'notes-packaging-controls-01');
...
for(const [id,parts,locale] of [['unchanged-ascii',entries,false],['unchanged-locale',entries,true],['reordered-ascii',changedEntries,false]]){
 ...
 records.push({id,file,sha256:sha(bytes),originalBytesIdentical:bytes.equals(original),changedParts,partOrder:Object.keys(archive)});
}
