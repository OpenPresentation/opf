...
if plan['path']: check('external action plan',lambda: eq(sha(plan['path']),plan['sha256']))
...
check('edited and reopened rasters identical',lambda: eq([r['sha256'] for r in report['phases']['edited']['rasters']],[r['sha256'] for r in report['phases']['reopened']['rasters']]))
...
target=run/'independent-native-audit.json'
