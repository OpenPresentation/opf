...
const reportPath = path.join(here, 'registry-reimport-03.analysis.json');
const importPath = path.join(here, 'registry-reimport-03.opf.json');
...
async function main() {
  ...
  try {
    ...
    const importerImageMatchesNativeCurrent = currentImageHash === afterPicture.mediaSha256 && currentImageHash === imageHash(previousImage);
    const altConsistent = (currentImage?.alt ?? '') === afterPicture.alt;
    evidence.importComparison = {sameNonTarget, priorImageMatchesInput, importerImageMatchesNativeCurrent, altConsistent,
    ...
    record('reimported non-target OPF content is unchanged and current picture is not stale',
      sameNonTarget && priorImageMatchesInput && importerImageMatchesNativeCurrent && altConsistent,
      evidence.importComparison, 'all non-target imported content matches; prior target matches input media; current target still matches lifecycle-saved media and alt');
  } catch (error) {
