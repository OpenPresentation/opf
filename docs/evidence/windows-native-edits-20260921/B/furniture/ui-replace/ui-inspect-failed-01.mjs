...
const importOutput = path.join(here, 'ui-import.opf.json');
const comparisonOutput = path.join(here, 'ui-comparison.json');
const checks = [];
const failures = [];
const evidence = {};
let manifest = null;
...
function childTags(node, parser, slidePart, archive) {
  const tagsRelId = node?.['p:nvPicPr']?.['p:nvPr']?.['p:custDataLst']?.['p:tags']?.['@_r:id'];
  ...
  for (const slidePart of slideParts) {
    ...
    for (const type of ['pic', 'sp', 'graphicFrame', 'cxnSp']) {
      ...
      for (const node of asArray(tree[key])) {
        ...
        const tagInfo = type === 'pic' ? childTags(node, parser, slidePart, archive) : {relationshipId: null, relationshipTarget: null, relationshipType: null, tags: []};
...
function withoutTargetPicture(slides, target) {
  ...
  for (const [slide, records] of Object.entries(slides)) {
    normalized[slide] = records.filter(record => !(slide === 'ppt/slides/slide2.xml' && record.type === 'pic' && record.id === target.id));
  }
  return normalized;
}

function currentFurnitureImage(document) {
  return document?.slides?.[1]?.localDesign?.header?.left?.image ?? null;
}
...
function normalizeImportedNonTarget(document) {
  ...
  const target = copy?.slides?.[1]?.localDesign?.header?.left;
...
if (fflate && parser && inputBytes && editedBytes) {
  try {
    ...
    record('original media bytes match the fixture image and change is localized', inputTarget.media?.sha256 === manifest?.inputs?.image?.sha256 &&
      editedTarget.media?.sha256 === sha(replacementBytes) && mediaChanges.length === 1,
      {originalTargetSha256: inputTarget.media?.sha256, expectedOriginalSha256: manifest?.inputs?.image?.sha256,
        changedMediaParts: mediaChanges}, 'one target media part changed from fixture image to replacement PNG');
    ...
    const tagRelationshipPreserved = inputTarget.tagRelationship.target === editedTarget.tagRelationship.target &&
      inputTarget.tagRelationship.type === editedTarget.tagRelationship.type;
    ...
    evidence.tag = {before: {relationship: inputTarget.tagRelationship, tag: tagBefore},
      ...
      tagHexPreserved, tagRelationshipPreserved, tagRelationshipIdPreserved};
    record('OPF_FURNITURE_V1 relationship target and hex payload retained', tagHexPreserved && tagRelationshipPreserved,
      {tagHexPreserved, tagRelationshipPreserved, tagRelationshipIdPreserved,
      ...
      'same tag part target and exact OPF_FURNITURE_V1 hex value');
...
if (beforeImport && currentImport && inputTarget && editedTarget) {
  ...
  evidence.import = {
    currentImageLocation: 'slides[1].localDesign.header.left.image',
  ...
  evidence.importedOtherContent = {sameNonTargetImport, originalSlideCount, currentSlideCount,
    beforeSlideSummaries: (beforeImport.slides ?? []).map(slide => ({title: slide.title, section: slide.section, blocks: slide.blocks, localDesign: slide.localDesign, effectiveHeader: slide.effectiveHeader, effectiveFooter: slide.effectiveFooter})),
    currentSlideSummaries: (currentImport.slides ?? []).map(slide => ({title: slide.title, section: slide.section, blocks: slide.blocks, localDesign: slide.localDesign, effectiveHeader: slide.effectiveHeader, effectiveFooter: slide.effectiveFooter}))};
