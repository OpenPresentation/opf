...
function Invoke-InventoryPureRegression {
    ...
    foreach($functionName in @('Get-InventoryContentGate','Get-InventoryFontGate','Get-InventoryParentDecision','Assert-InventoryParentSuccess','Invoke-InventoryCom','Test-InventoryPathEqual','Read-InventoryTextRange','Close-OwnedInventoryPresentation')) {
...
if(-not $Worker) {
    ...
    if(Test-Path -LiteralPath $registrationPath) { try {$registrations=@(Get-Content -LiteralPath $registrationPath -Raw -Encoding UTF8 | ConvertFrom-Json)} catch {$parentFailure="Unreadable font-registration.json: $($_.Exception.Message)"} }
