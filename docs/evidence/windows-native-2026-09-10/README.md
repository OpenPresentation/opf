...
The accepted-text and complete selected-workbook chart matrices now have successful real PowerPoint evidence on both pinned Node runtimes. Both runtimes also reproduce the eight metric tab failures and the portrait/right ink overflow. Metric fixes and broader native coverage remain open. This is a bounded Windows testing checkpoint, not an export-equivalence or release approval.
...
| Full chart matrix, both Node runtimes | Eight separately selected actual workbook edits per runtime; 16 successful 45-second bounded helpers. All 384 slide imports and 384 independent chart-cache comparisons pass. All 128 original/reopened PNG pairs match; each selected edited slide changes while the other seven remain unchanged. Exact live edited categories/series/values pass. `chart-two-runtime-summary.json` binds all 16 raw attempts. |
| Native-only pie control | `raw/pie-native-created-control-03` creates a pie with PowerPoint `AddChart2`, using no OPF input. Its categories are real strings before saving and null elements after reopening the same valid one-slide deck. Null loaded-pie getters are recorded as unavailable; exact live edited categories and independent persisted cache checks remain required. Controls 01/02 preserve an initial empty-deck save mistake and its detected failure. |
...
Full chart-edit coverage is complete for slides 1–8 on both Node versions, using exact verifier bytes in `verifiers/chart-matrix` from [PPTX #21](https://github.com/OpenPresentation/opf-pptx/pull/21). Loaded-pie `Series.XValues` remains unavailable on this Office build and is never called matching category evidence. The original chart fixture requests unmeasured Roboto, so these chart checks are not font-fidelity evidence. Metric tab discrepancies above 0.02 points and the portrait/right Latency ink counterexample were reproduced unchanged on both runtimes and remain unresolved. Original and native-saved tab coordinates remain precise in DrawingML (`raw/metric-node24-bounded-01/tab-xml-diagnostic.json`); this does not establish whether native shaping, character-bound reporting or both explain the discrepancy. The Mac owner is investigating the shared metric/font runtime; this Windows change does not alter it. Representative tables, quotes, code, images, editing/undo and repeated exports remain additional work.

Intermediate chart attempts remain raw: `chart-node24-slide2-observed` fails the null getter criterion; `chart-node24-slide2-live` additionally captures correct live edited categories before closing the workbook; `chart-node24-final-slide2` fails an initial independent parser on package-absolute chart relationships. Their generation-bound verifiers are retained. `chart-node20-matrix-slide1` records a parent command argument error before any Office worker started. Corrected Node 20 attempts use the fresh `matrix02` directories. These failures are not relabelled as passes.
...
For charts, generate a fresh directory with `node test/native-chart-colors.mjs generate artifacts/chart-new-slide1`, invoke `powershell.exe -NoProfile -NonInteractive -File test/native-chart-colors.ps1 -EvidenceDirectory artifacts/chart-new-slide1 -EditSlide 1`, then run `node test/native-chart-colors.mjs compare artifacts/chart-new-slide1`. Continue with separately generated directories for each distinct slide 2–8 and each Node runtime. See the PPTX PR's `docs/native-chart-checks.md` for exact gates. To repeat the independent native-created control, copy `raw/pie-native-created-control-03/verifier.ps1` to the PPTX checkout's `artifacts/native-pie-control.ps1`, create a fresh output directory, then invoke that script with `-EvidenceDirectory` selecting it; the script uses `test/native-process.ps1` and never probes `Series.Formula`.
...
node docs/evidence/windows-native-2026-09-10/verify-chart-matrix.mjs
```
