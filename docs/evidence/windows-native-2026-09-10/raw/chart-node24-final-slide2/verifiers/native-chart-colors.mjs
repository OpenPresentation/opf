...
function readChartCache(archive,index) {
 ...
 const part=path.posix.normalize(path.posix.join('ppt/slides',rel.Target)),plot=xml(part)['c:chartSpace']['c:chart']['c:plotArea'];
 const series=array((plot['c:barChart']??plot['c:pieChart'])?.['c:ser']);assert.ok(series.length);
 const points=cache=>{assert.ok(cache);const points=array(cache['c:pt']).sort((a,b)=>Number(a.idx)-Number(b.idx));assert.equal(points.length,Number(cache['c:ptCount'].val));return points.map(point=>point['c:v']);};
 return series.map(series=>({name:points(series['c:tx']['c:strRef']['c:strCache'])[0],categories:points(series['c:cat']['c:strRef']['c:strCache']),values:points(series['c:val']['c:numRef']['c:numCache']).map(Number)}));
}
const runtime={};
