...
if(mode==='generate'){
 ...
 for(const phase of ['original','reopened','edited']){assert.equal(native[phase].length,8);for(const [i,slide]of native[phase].entries()){
  ...
  assert.deepEqual(slide.seriesData,data.columns.slice(1).map((name,index)=>({name,categories:data.rows.map(row=>row[0]),values:data.rows.map(row=>row[index+1])})),`${phase} slide ${i+1}: actual native series data`);
