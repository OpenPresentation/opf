...
function Invoke-TabHelperPureRegression {
    ...
    foreach($functionName in @('Invoke-ControlCom','Add-ControlTextBox')) {
...
if([string]::IsNullOrWhiteSpace($OutputDirectory)) { throw 'OutputDirectory is required unless -PureRegression is selected' }
