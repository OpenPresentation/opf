...
function Add-ControlTextBox($Shapes,[string]$Name,[string]$Text,[double]$Left,[double]$Top,[string]$Prefix) {
    $shape=Invoke-ControlCom "$Prefix.addTextbox" { return ,$Shapes.AddTextbox(1,$Left,$Top,500,20) }
    Invoke-ControlCom "$Prefix.name.set" { $shape.Name=$Name }
    ...
    Invoke-ControlCom "$Prefix.text.set" { $range.Text=$Text }
...
function Read-RangeBounds($Range,[string]$Prefix) {
    ...
    return [ordered]@{left=$left;top=$top;width=$width;height=$height}
}
...
function Read-RangeFont($Range,[string]$Prefix) {
    ...
    return [ordered]@{name=$name;size=$size}
}
...
function Read-ControlObservation($Presentation,[string]$Phase) {
    ...
    for($index=0;$index -lt $targets.Count;$index++) {
        ...
        $records+=@([ordered]@{
            ...
            tabStopPoints=$tabStopPosition
            tabShapeName=$tabName
            literalShapeName=$literalName
            tabShapeLeft=$tabLeft
            tabShapeTop=$tabTop
            literalShapeLeft=$literalLeft
            literalShapeTop=$literalTop
    ...
    return [ordered]@{phase=$Phase;slideCount=$slideCount;shapeCount=$shapeCount;slideWidth=$slideWidth;slideHeight=$slideHeight;records=$records}
}
...
function Get-PhaseMetrics($Observation,[double]$Tolerance) {
    ...
    $maximumTabError=($records | Measure-Object -Property tabErrorPoints -Maximum).Maximum
    $maximumLiteralError=($records | Measure-Object -Property literalErrorPoints -Maximum).Maximum
    $maximumPairDelta=($records | ForEach-Object {[Math]::Abs([double]$_.tabMinusLiteralPoints)} | Measure-Object -Maximum).Maximum
    return [ordered]@{
        phase=$Observation.phase
        tolerancePoints=$Tolerance
        tabGatePassed=(@($records | Where-Object {-not $_.tabGatePassed}).Count -eq 0)
        literalGatePassed=(@($records | Where-Object {-not $_.literalGatePassed}).Count -eq 0)
        pairAgreementGatePassed=(@($records | Where-Object {-not $_.pairAgreementGatePassed}).Count -eq 0)
...
function Get-PersistenceMetrics($Original,$Reopened,[double]$Tolerance) {
    ...
    for($index=0;$index -lt $Original.records.Count;$index++) {
        ...
        $records+=@([ordered]@{index=$index;maximumDeltaPoints=($deltas.Values | Measure-Object -Maximum).Maximum;deltas=$deltas})
    }
