...
try {
    ...
    @{observations=$observations;powerPointBuild=(Get-Item -LiteralPath $exe).VersionInfo.FileVersion;executableSha256=(Sha $exe);windowsBuild="$($os.CurrentBuild).$($os.UBR)";hostVersion=$PSVersionTable.PSVersion.ToString();verifierSha256=(Sha $PSCommandPath);workerVerifierSha256=(Sha (Join-Path $PSScriptRoot 'native-process.ps1'));fontSha256=(Sha (Join-Path $env:WINDIR 'Fonts/calibri.ttf'));pptxSha256=(Sha $source);scope='Native-created tab stops and literal text at the same requested offsets. TextRange2 bounds and native raster observations only; no OPF input and no adjusted tolerance.'} | ConvertTo-Json -Depth 15 | Set-Content -LiteralPath (Join-Path $root 'control.json') -Encoding UTF8
