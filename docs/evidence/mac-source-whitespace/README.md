...
`source-graph.json` pins the four product commits. Subsequent branch commits contain only CI coordination and evidence. `candidate-manifest.json` records the unpublished local tarballs. Installed browser reports bind consumer locks, bundles and font assets; the scalar workflow also records its verifier and bundle hashes. No package was published and no deployment changed.
