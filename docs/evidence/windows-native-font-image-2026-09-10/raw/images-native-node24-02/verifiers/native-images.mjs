...
const options={imageFormat:'compatible',strictAssets:true};
if(mode==='generate'){
 ...
 for(const [id,document]of [['images-fit',source],['images-crop',{...source,design:{...source.design,imageFill:'crop'}}],['images-editor-redo',edited]]){
  ...
  for(const [i,slide]of document.slides.entries()){
   ...
   const relationship=picture['p:blipFill']['a:blip']['r:embed'];
   const relationships=find(parser.parse(new TextDecoder().decode(zip[`ppt/slides/_rels/slide${i+1}.xml.rels`])),'Relationship');
   const target=relationships.find(r=>r.Id===relationship).Target;
   const media=target.startsWith('/')?target.slice(1):path.posix.normalize(path.posix.join('ppt/slides',target));
   const embedded=Buffer.from(zip[media]);
   if(meta.format==='webp'){
    assert.equal((await sharp(embedded).metadata()).format,'png');
    assert.deepEqual(await sharp(embedded).ensureAlpha().raw().toBuffer(),await sharp(sourceBytes).ensureAlpha().raw().toBuffer(),'Compatible WebP PNG keeps decoded source pixels');
   }else assert.deepEqual(embedded,sourceBytes,'Already-compatible source bytes preserved');
   records.push({slide:i+1,alt:slide.image.alt,sourceSha256:sha(sourceBytes),embeddedSha256:sha(embedded),sourceUri:slide.image.src,sourceWidth:meta.width,sourceHeight:meta.height,transcodedToPng:meta.format==='webp',fill,cell,box,content});
  }
  decks.push({id,pptxSha256:sha(bytes),sourceSha256:sha(await readFile(path.join(root,id+'.opf.json'))),records});
 }
 await write('generation.json',{node:process.version,runtime,imageFormat:'compatible',decks,scope:'Nine raster formats/aspects in fit/crop plus an atomic editor source replacement and undo/redo. WebP uses the default compatible PNG conversion with exact decoded source pixels. Native alt edits and current embedded image-byte imports are distinct from arbitrary picture-position/crop round trips.'});
}else{
  ...
  for(const suffix of ['', '-saved','-edited']){
   ...
   for(let cycle=0;cycle<3;cycle++){
    ...
    for(const [index,slide]of restored.slides.entries()){
     assert.equal(slide.blocks.length,1);assert.equal(slide.blocks[0].type,'image');const image=slide.blocks[0].image;assert.equal(image.alt,(suffix==='-edited'?'Native alt ':'')+deck.records[index].alt);assert.equal(sha(Buffer.from(image.src.split(',')[1],'base64')),deck.records[index].embeddedSha256);
    }
