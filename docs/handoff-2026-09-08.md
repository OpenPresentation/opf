...
## September 9 UTC — three production deployments and security policies verified
...
pptx.dev draft PR #15 remains on `codex/published-ecosystem-adoption-20260908`. Candidate `2dae8309517780eaa20ea8465d95d190d4db54cd` passed 591 tests, typecheck and inspector E2E on Linux and Windows (CI `34303591013`) and Bugbot, but failed Vercel packaging. A complete build at `8e14683aa1c65427881d9703555c81d6d99e3e4f` passes locally but exposed the Next 16.3 standalone/adapter conflict in Vercel. Conditional standalone output and root SDK lockfile consolidation are being verified on that same PR; fetch its latest head. Do not merge until renewed CI/review and preview deployment/browser checks pass. Its primary renderer/exporter adoption remains incomplete.
...
Remaining work: standalone renderer/PPTX/editor Dependabot/security rollout; pending application action PRs; free browser PPTX import/export controls and repeatable complete workflows; actual published renderer/editor/exporter integration in pptx.dev; broader native PowerPoint comparisons. The public npm PPTX dependency graph still includes PptxGenJS's unused vulnerable image-size dependency; the tested application-only override does not fix other npm consumers. Preserve website PR #4 and pptx.dev PR #6. Native PowerPoint evidence remains three targeted slides, not broad pixel equivalence.
