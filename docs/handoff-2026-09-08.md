...
Core PR #26 now updates the complete release plan and immutable CI refs; renewed coordinated CI/review remains required before its merge. The separate `codex/skills-installer-20260908` branch prepares CLI 0.5.0 with bundled, local-edit-safe installation of all six skills; it is not published. Public application upgrades, security remediation, website changelog/deployment and full deployed E2E coverage remain open. See [current Windows release/adoption evidence](evidence-2026-09-08-windows.md). Historical checkpoints below are superseded where they describe unpublished downstream packages or stale lockfiles.
