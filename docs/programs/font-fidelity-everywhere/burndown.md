...
| FF-29 | Layout catalog parity and export fidelity | opf, opf-pptx, pptx-gallery | FF-23 | todo |  |
| FF-30 | Content blocks keep metric text in preview and export | opf, opf-render, opf-pptx | FF-23 | todo |  |
| FF-31 | Font availability for developers everywhere; substitution never rewrites export | opf-render, opf-pptx, opf | FF-23, FF-35 | todo |  |
...
Every item from FF-24 to FF-36 proves its result by re-running the FF-23 audit
on the fix heads (or the merged heads) and updating
`gallery-support/support-status.json` and the affected rows of
[gallery-support.md](gallery-support.md) in the same or a follow-up PR.
...
**FF-31 Font availability.**
- Every openly licensed scheme family (30 upstream and 4 legacy schemes today)
  is bundled by opf-render or fetchable through a documented caller-supplied
  registry recipe (package, version, hash, license), with a test that the
  recipe resolves every such family.
- For the default `aptos` scheme (FF-35) the docs state that Aptos and
  Aptos Display are not openly licensed and cannot be bundled, what export
  does without them, what the preview does (recorded substitution) and how a
  caller supplies licensed faces.
- [Font fidelity](../../font-fidelity.md) documents strict-mode behaviour
  (which calls throw `font-unavailable` or `missing-glyph`, and when) and
  cloud/serverless guidance for export, preview and raster output with no
  system fonts.
- Substitution never rewrites the chosen font in exported PPTX. With any
  registry and substitution policy, `toPptx` writes the chosen families and
  only reports substitutions. Tests cover the 7 substituting schemes (`aptos`,
  `calibri`, `consolas`, `courier-new`, `georgia`, `tahoma`,
  `times-new-roman`) and the `minimal` theme.
- Audit evidence: "export w/ office registry" equals the chosen family for all
  93 font schemes, and open families no longer fail the registry probe.
