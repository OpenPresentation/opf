...
const root=fileURLToPath(new URL('../',import.meta.url)),prefix='docs/evidence/native-resume/';
...
assert.equal(offset,result.stdout.length);console.log(`Verified all ${summary.artifacts.length} native-resume artifacts against actual ${ref||'staged'} Git bytes.`);
