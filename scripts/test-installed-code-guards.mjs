// Fail closed on changed candidate bytes, integrity, source links and loaders.
// Only temporary candidate installation artifacts are touched, then restored.
...
const consumer=await realpath(path.join(out,'consumer'));
...
const reject=(directory,pattern,env=process.env)=>{
  const result=spawnSync(process.execPath,[script,directory],{cwd:root,env,encoding:'utf8'});
...
try {
  ...
  await writeFile(lockPath,JSON.stringify(lock));reject(consumer,/Candidate integrity must match the installed lock/);
