...
try {
  ...
  await page.route('**/*',route=>{
    ...
    if(url==='http://opf-fixture.local/')return route.fulfill({contentType:'text/html',body:'<!doctype html><div id="canvas" style="max-width:1100px"></div><pre id="results"></pre>'});
  ...
  await page.goto('http://opf-fixture.local/');await page.addScriptTag({type:'module',content:bundle});
  ...
  const report={node:process.version,browser:browser.version(),title,errors,unexpectedRequests,bundleSha256:hash(bundle),fontsSha256:hash(fonts),inputs,
    scope:'Existing rich-text canvas harness executed in real Edge with bundled font bytes and intercepted local fixture requests. DOM selection/input/composition events test formatting, typing, draft undo/redo, session undo, cancellation, concurrency, IME guards and caret geometry. This is linked-source browser behavior, not native IME or PowerPoint fidelity.'};
