...
const root=fileURLToPath(new URL('../../',import.meta.url)),repo=process.argv[2],major=process.versions.node.split('.')[0];
const output=path.join(root,'text-raster-final-checks');mkdirSync(output,{recursive:true});
const commands={
 opf:[['pnpm','--filter','@openpresentation/opf','test'],['pnpm','typecheck'],['pnpm','lint'],['pnpm','check:examples']],
 'opf-editor':[['npm','run','typecheck'],['npm','run','validate'],['npm','test']],
...
for(const [i,[name,...args]]of commands[repo].entries()){
 ...
 const result=spawnSync(invocation.command,invocation.args,{cwd:path.join(root,repo),encoding:'utf8',maxBuffer:30*1024*1024,env:{...process.env,OPF_GOLDEN_OUT:path.join(output,`golden-node${major}`),OPF_GOLDEN_ARTIFACTS:'1',OPF_TEXT_OUT:path.join(output,"pptx-node"+major)}});
