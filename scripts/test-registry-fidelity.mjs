...
const atLeast=(name,minimum)=>{
 const actual=plan.packages.find(item=>item.name===name).version.split('.').map(Number);
 for(let index=0;index<3;index++)if(actual[index]!==minimum[index])return actual[index]>minimum[index];
 return true;
};
