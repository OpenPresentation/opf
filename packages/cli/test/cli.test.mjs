...
describe("opf validate", () => {
  ...
  test("rejects an invalid deck with a non-zero exit code and reports errors", () => {
    ...
    // Failure output (unlike the success path) is written to stderr.
  ...
  test("reports a read/parse error and exits 1 for a nonexistent file", () => {
...
describe("opf usage", () => {
  test("prints usage and exits 2 with no arguments", () => {
    const result = runCli([]);
    assert.equal(result.status, 2);
    assert.match(result.stderr, /Usage:/);
    assert.match(result.stderr, /opf validate <file>/);
    assert.match(result.stderr, /opf catalogs/);
    assert.match(result.stderr, /opf schemas/);
  });
