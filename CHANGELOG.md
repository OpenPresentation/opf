...
- Record production issue88 features (Inspector overlay/json-options, gallery Playground+Editor links, Header & footer playground example) live on `www.pptx.dev` / `www.pptx.gallery` / `www.openpresentation.org` (squash `17de6da` / `c7d3754` / `ea0d032`). GitHub [issue 88](https://github.com/OpenPresentation/opf/issues/88) stays open. Renderer [issue 24](https://github.com/OpenPresentation/opf-render/issues/24) and native `p:hf` [issue 87](https://github.com/OpenPresentation/opf/issues/87) stay not shipped. Geometry drafts and homepage-renderer are not described as shipped. The developer-ready milestone is not complete.
