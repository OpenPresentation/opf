...
- FF-26: `composeSlide` resolves `design.slideImage` into `geometry.slideImage` with a position, a crop/fit frame and a region, so preview and export place the same slide-level image. A slide's own `design.slideImage` turns it on. A deck-level value applies only where the slide's layout declares `slideImage: true`. Banded positions give the image half the slide, and headings and content compose in the other half. A root image with the same source becomes the slide image instead of a duplicate content item. Among the bundled examples, only `technical/slide-design-overrides` slide 4 changes geometry: it sets a slide-level right image. No schema or package version changes.
