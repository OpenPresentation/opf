...
- Add a language/script font model (font-fidelity-everywhere FF-18). `resolveScriptFonts()` resolves a presentation's language and effective font scheme into heading and body fonts for the OOXML `latin`, East Asian and complex-script slots, plus the BCP-47 `lang`, ISO 15924 script, direction/RTL and a per-script supplemental theme font. It is exported with `scriptFontRole()`, for PowerPoint or Google Slides targets. All 93 bundled language records now state `script` and `direction`. `FontScheme` (catalog records and `design.fontScheme`) accepts optional `eastAsian`/`complexScript` `{ major, minor }` slots. Every change is additive: existing documents and catalogs keep validating, and renderer/exporter output is unchanged until they adopt the resolver (FF-19, FF-07). See [the design note](docs/programs/font-fidelity-everywhere/script-font-model.md).
