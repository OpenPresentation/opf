...
- **Breaking** — add content color references: `TextRun.color`, styled table cell `fill`/`color`, and cell border `color` accept a color-scheme slot name (`accent1`–`accent6`, `dark1/2`, `light1/2`, `hyperlink`, `followedHyperlink`), an abstract role name (`primary`, `secondary`, `accent`, `background`, `surface`, `text`, `textSecondary`), or a `var:<id>` reference, in addition to literal hex. The shared `ColorRef` definition constrains these fields, so arbitrary non-hex strings that previously slipped through `TextRun.color` as undefined behavior (`"red"`, `"rgb(…)"`) are now schema errors — see [`docs/migrations/color-references.md`](docs/migrations/color-references.md); this must ship in a minor release. The hex-only pattern on styled table cells is widened. Prefer names over hex so styled content survives re-theming.
