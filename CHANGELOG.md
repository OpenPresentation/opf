...
- Reduce the chart-type catalog to the chart types Aspose.Slides officially supports: one non-deprecated record per Aspose.Slides `ChartType` (25 records), each naming its type in `mappings.renderers["aspose-slides"].chartType`. Deprecate the other 51 records (series-count, progress and region-map duplicates plus bullet, sparkline, dot-plot, dumbbell, high/low and bar-plus-line Pareto compositions) with a `deprecation.replacedBy` id rather than removing them, because removing a published catalog record is breaking. Deprecated ids still resolve and validate, but the validator warns and names the replacement, `opf catalog` hides them unless `--all` is passed, and the example generator no longer picks them. The committed example corpus is pinned by the renderer golden baseline, so its remaining deprecated ids migrate with the coordinated 0.12.0 removal; until then the examples test tolerates only deprecation warnings. They are scheduled for removal in 0.12.0; see [migration notes](docs/migrations/0.12.0.md) and the [Aspose.Slides chart-type reference](docs/programs/font-fidelity-everywhere/aspose-chart-types.md). Exporter and renderer output is unchanged.
